...
| 2026-03-13 | All message generation functions (`generateFollowUp`, `generateInMail`, `generateInboxReply`, `generatePostComment`) used ambiguous "writing for ${accountConfig.name}" framing — Claude interpreted the sender as the recipient and addressed messages to the sender (e.g. "Nicole," as opener). Also present in onboarding template generation. Affected all profiles since launch. | 2026-04-07 | Fixed: added explicit SENDER/RECIPIENT labeling to all 5 functions in messenger.js; fixed onboarding template prompt in webhook-server.js; added sender-name sanity check on template generation |
...
| Date Found | Description | Status | Resolved Date |
|------------|-------------|--------|---------------|
| 2026-04-07 | [chris] inmails: running inmails phase | Investigating | — || 2026-04-07 | [darren] inmails: running inmails phase | Investigating | — || 2026-04-07 | [nicolepindul] inmails: running inmails phase | Investigating | — |
